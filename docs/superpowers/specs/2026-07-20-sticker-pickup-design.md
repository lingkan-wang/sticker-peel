...
### PEELING
沿用现有 `PeelState` 逻辑。每帧在 `step()` 之后检查 `peel / maxPeel > DETACH_THRESHOLD`（0.75）；越线立即转 HELD，并把当时的光标位置记为抓取锚点。

未越线时松手 → `PeelState` 的弹簧回弹 → `peel` 归零后转回 ATTACHED。

### HELD
- `peel` 用弹簧收敛到 `maxPeel * HELD_CURL`（0.18），保留可见卷边而不抹平。
- `pos` 用临界阻尼弹簧追光标（`stiffness 0.18 / damping 0.82`），刻意保留约 60ms 滞后 —— 这个滞后就是惯性感的来源，不是延迟 bug。
